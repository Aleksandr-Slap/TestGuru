...
users = User.create!([{ name: 'Sasha',
                    password: 123456,
       password_confirmation: 123456,
                       email: 'aleksandrslap@gmail.com' }])

p "Created #{User.count} users"

Test.destroy_all

tests = Test.create([{ title: 'Ruby', level: 1, category_id: categories[1].id, user_id: users[0].id },
...
questions = Question.create([{ body: 'What is HTML?', test_id: tests[2].id },
...
answers = Answer.create([{
                         ...
                         { body: "Database management system",
                           question_id: questions[0].id,
                           correct: false
                         },
                         { body: "Matz",
